...
# double selection ############################################################
Data <- mydata1[,list(id,y,age,gender,race,A)] # drop all mediators
setnames(Data,old="y",new="Y")
Data_Monly <- mydata1_genes # mediators only
...
if (!file.exists(file="dataex-boot_ds-obs.Rdata")) {
  ...
  save(res,file="dataex-boot_ds-obs.Rdata")
}
...
save(res,res.ds,file="dataex-boot_ds-obs.Rdata")
q()
